...
public partial class CompressionZStd : ICompressionFormat
{
        ...
        CompressionFlags inFlags = CompressionFlags.None) where T : unmanaged
    {
        ...
        if (inFlags.HasFlag(CompressionFlags.ZStdUseDicts))
        {
            ...
            {
                var ebxDict = FileSystemManager.GetFileFromMemoryFs("Dictionaries/ebx.dict");
    ...
    public unsafe void Compress<T>(Block<T> inData, ref Block<T> outData,
        CompressionFlags inFlags = CompressionFlags.None) where T : unmanaged
    {
        var code = ZSTD_compress((IntPtr)outData.Ptr, outData.Size, (IntPtr)inData.Ptr, inData.Size);
        GetError(code);
    }
