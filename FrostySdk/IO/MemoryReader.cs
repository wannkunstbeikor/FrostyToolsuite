...
public class MemoryReader : IDisposable
{
    ...
    public long ReadLong(bool pad = true)
    {
        ...
        return ((long)(uint)(m_buffer[4] | (m_buffer[5] << 8) | (m_buffer[6] << 16) | (m_buffer[7] << 24)) << 32) |
               (uint)(m_buffer[0] | (m_buffer[1] << 8) | (m_buffer[2] << 16) | (m_buffer[3] << 24));
    }
    ...
    public Guid ReadGuid(bool pad = true)
    {
        ...
        return new Guid(new[]
        {
            ...
            m_buffer[8], m_buffer[9], m_buffer[10], m_buffer[11], m_buffer[12], m_buffer[13], m_buffer[14],
            m_buffer[15]
        });
    }
    ...
    {
        var outBuffer = new byte[numBytes];
    ...
    public unsafe IList<long> Scan(string pattern)
    {
        ...
        var bytePattern = new PatternType[pattern.Length / 2];
        for (var i = 0; i < bytePattern.Length; i++)
        {
            var str = pattern.Substring(i * 2, 2);
            bytePattern[i] = new PatternType
            {
                IsWildcard = str == "??",
                Value = str != "??" ? byte.Parse(pattern.Substring(i * 2, 2), NumberStyles.HexNumber) : (byte)0x00
            };
        }
        ...
        var pos = Position;
        var buf = ReadBytes(1024 * 1024);
        var startPtr = buf == null ? null : (byte*)Marshal.UnsafeAddrOfPinnedArrayElement(buf, 0);
        var ptr = startPtr;
        var endPtr = ptr + (1024 * 1024);
        var tmpPtr = ptr;
