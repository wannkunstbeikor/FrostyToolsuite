...
/// <summary>
///     A <see cref="HuffmanNode" /> with additional integer for how many times this node was encountered in the data to
///     encode. This is used to construct the huffman tree.
/// </summary>
internal class HuffManConstructionNode : HuffmanNode, IComparable<HuffManConstructionNode>
{
    ...
    {
        var cmp = Occurrences.CompareTo(other?.Occurrences);
        if (cmp == 0)
        {
    ...
    {
        var ld = Left?.GetRemainingDepth() ?? 0;
        var rd = Right?.GetRemainingDepth() ?? 0;
...
///     This is used as part of the return value of the encoding method. The identifier is to be the identifier of a string
///     or text, with the coupled position being the bit offset in the encoded byte array.
/// </summary>
...
/// <summary>
///     Return value of the encoding function. This contains the encoded texts as byte array, as well as the list of
///     <see cref="IdentifierPositionTuple{T}" /> that detail which text is at what bit offset inside the array.
/// </summary>
...
public class HuffmanEncodedTextArray<T>
{
    ...
    /// <summary>
    ///     The list of string identifiers, and the bit position of the text for the identifier inside the
    ///     <see cref="EncodedTexts" /> byte array.
    /// </summary>
...
public class HuffmanEncoder
{
    ...
    /// <summary>
    ///     Uses the given input to construct the huffman encoding table (or tree). The created encoding includes end delimiter
    ///     character (char 0x0) with a number of occurrences of the number of given strings.
    ...
    public IList<uint> BuildHuffmanEncodingTree(IEnumerable<string> texts)
    {
        ...
        var rootNode = CalculateHuffmanEncoding(strings);

        var encodingNodes = GetNodeListToWrite(rootNode);
    ...
    /// <summary>
    ///     Encodes the given text into a bool values, using the encoding set previously by
    ///     <see cref="HuffmanEncoder.BuildHuffmanEncodingTree(IEnumerable{string})" />.
    /// </summary>
    /// <param name="text">A single text to encode.</param>
    /// <param name="includeEndDelimeter">
    ///     Whether or not the encoding for a 0x0 character should be added to the end of the
    ///     text.
    /// </param>
    /// <returns>the bool representation of the encoded text.</returns>
    /// <exception cref="System.Collections.Generic.KeyNotFoundException">
    ///     If a character or symbol to encode was not found in
    ///     the dictionary
    /// </exception>
    ...
    /// <summary>
    ///     Encodes the given String using the previously created Huffman Encoding from
    ///     <see cref="HuffmanEncoder.BuildHuffmanEncodingTree(IEnumerable{string})" /> and returns the created byte array
    ///     together with the list of text identifiers and their bit offsets in the byte array.
    /// </summary>
    ...
    /// <param name="compressResults">
    ///     If true, then this method tries to reuse already compiled string encodings and produced
    ///     bit segments, so the returned byte array might be shorter. Defaults to false.
    /// </param>
    /// <returns>
    ///     An instance of <see cref="HuffmanEncodedTextArray{T}" /> with the byte array of the encoded texts, and a list
    ///     of the given text identifiers and their bit position inside the byte array. The list has the same ordering as the
    ///     given input to this method.
    /// </returns>
    /// <exception cref="System.Collections.Generic.KeyNotFoundException">
    ///     If a character or symbol to encode was not found in
    ///     the dictionary
    /// </exception>
    /// <exception cref="System.InvalidOperationException">If no encoding has been created yet.</exception>
    public HuffmanEncodedTextArray<T> EncodeTexts<T>(IEnumerable<Tuple<T, string>> textsPerIdentifier,
        bool compressResults = false)
    {
        CheckEncodingExists();
        ...
        {
            var textIdentifier = textWithIdentifier.Item1;
            var text = textWithIdentifier.Item2;
            var position = encodedTextBools.Count;

            var encodeText = true;
            if (compressResults)
            {
                var exists = alreadyEncodedTextPositions.TryGetValue(text, out var existingPos);
                if (!exists)
                {
        ...
        var byteSize = (encodedTextBools.Count / 8) + 1;
    ...
    private static HuffManConstructionNode CalculateHuffmanEncoding(IList<string> texts)
    {
        ...
        foreach (var text in texts)
        {
            ...
            {
                if (charNumbers.TryGetValue(c, out var occurences))
                {
    ...
    private static IDictionary<char, IList<bool>> GetCharEncoding(IList<HuffmanNode> encodingNodes)
    {
        ...
        foreach (var node in encodingNodes)
        {
            if (node.Left == null && node.Right == null)
            {
                var c = node.Letter;
                var path = GetCharEncodingRecursive(node);
    ...
    private static IList<HuffmanNode> GetNodeListToWrite(HuffmanNode? rootNode)
    {
        ...
        // get all branches
        var branches = GetAllBranchNodes(new List<HuffmanNode> { rootNode });
    ...
    private static IList<bool> GetCharEncodingRecursive(HuffmanNode node)
    {
        ...
        var encoding = GetCharEncodingRecursive(parent);
    ...
    ///     the dictionary
    /// </exception>
    private static IList<bool> GetEncodedText(IEnumerable<char> toEncode, IDictionary<char, IList<bool>> charEncoding,
        bool includeEndDelimeter)
    {
        List<bool> encodedText = new();

        foreach (var c in toEncode)
        {
    ...
    {
        var found = charEncoding.TryGetValue(c, out var encodedChar);
        if (found)
        {
            return encodedChar!;
        }
        ...
        var errorMessage = $"Encoding does not contain a mapping for symbol of value {(int)c}: '{c}'!";
    ...
    private void CheckEncodingExists()
    {
        ...
        {
            throw new InvalidOperationException(
                "Cannot encode texts before the Huffman tree was built! Call 'BuildHuffmanEncodingTree' on the encoder before attempting to encode a string!");
        }
    }
}