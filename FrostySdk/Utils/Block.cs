...
{
    /// <summary>
    ///     Indicates if the underlying memory is assumed to be already disposed before <see cref="Block{T}.Dispose()" /> is
    ///     called.
    ///     This is the case with stack allocated memory, pointers from <see langword="fixed" /> scopes and Spans.
    /// </summary>
    private bool m_fragileMemory;

    /// <summary>
    ///     Indicates if the underlying memory is usable. Any operation with this disabled will cause the
    ///     <see cref="Block{T}" /> to destroy itself.
    /// </summary>
    ...
    public Block(T[] inArray)
    {
        fixed (T* ptr = inArray)
        {
            var size = inArray.Length * sizeof(T);
    ...
    ///     Thrown if the index is negative or larger than the size of this
    ///     <see cref="Block{T}" />.
    /// </exception>
    ...
    public void Resize(int inNewSize)
    {
        ...
        // Get the old offset.
        var currentOffset = ShiftAmount;
        var newSize = inNewSize * sizeof(T);
    ...
    /// </summary>
    /// <param name="inAmount">
    ///     Amount of elements to shift the <see cref="Block{T}" /> start by.
    ///     Can be negative, but attempting to shift before the beginning of the block will result in an exception.
    /// </param>
    /// <exception cref="ArgumentOutOfRangeException">
    ///     Thrown if attempting to shift before the beginning of the block (e.g.
    ///     BasePtr[-1]) or after the end of the block.
    /// </exception>
    public void Shift(int inAmount)
    {
        ...
        {
            throw new ArgumentOutOfRangeException(nameof(inAmount),
                "Attempted to shift before the beginning of the block!");
        }
    ...
    public Span<T> ToSpan(int inStart)
    {
        ...
        {
            throw new ArgumentOutOfRangeException(nameof(inStart),
                "Attempted to slice before the beginning of the block!");
        }
    ...
    public void CopyTo(Block<T> inDest)
    {
        ...
        {
            throw new ArgumentOutOfRangeException(nameof(inDest),
                "The target block must be at least as big as the source block!");
        }
    ...
    /// <summary>
    ///     Signals to the <see cref="Block{T}" /> that its underlying memory might already not exist anymore when
